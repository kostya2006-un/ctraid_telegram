...
class UserAPI(BaseServer):
    ...
    async def get_user_by_id(self, user_id: str) -> User | None:
        response = await self.client.get(f"/telegram/users/{user_id}/")
    ...
    async def get_users(self) -> UserList | None:
        response = await self.client.get(f"/telegram/users-list/")
        if response.status_code != 200:
            return None
    ...
    async def create_user(self, user: User) -> User | None:
        response = await self.client.post("/telegram/users/", json=user.model_dump())
    ...
    async def put_user(self, user: User) -> User | None:
        response = await self.client.put(
            f"/telegram/users/{user.user_id}/", json=user.model_dump()
        )
